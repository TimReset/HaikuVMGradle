// Is used (automatic) only when compiled from Arduino IDE. It's important to have an extension of cpp.
...
JNIEXPORT jboolean Java_processing_hardware_arduino_cores_arduino_HardwareSerialImpl_isOpen
  (JNIEnv *env, jclass obj)
{
	// We need the loop! For unknown reasons it can't be done with: return Serial
	while (!Serial) {
